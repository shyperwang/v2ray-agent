...
initTLSNginxConfig(){
    ...
    then
        ...
        if [[ -n `find /www/server |grep -v grep|grep -w nginx`]]
        then
        ...
        # 测试nginx
        echoContent yellow "\n检查Nginx是否正常访问"
        domainResult=`curl -s ${domain}/test|grep ${domain}`
        if [[ ! -z ${domainResult} ]]
        then
            handleNginx stop
...
server {
    listen 80;
    server_name ${domain};
    root /www/wwwroot/${domain}/html;
    location ~ /.well-known {allow all;}
    location /test {return 200 '${domain}';}
}
EOF
    else
...
#    echoContent yellow "添加伪装博客--->"
    ...
    if [[ ! -z `find /www/server |grep -v grep|grep -w nginx`]]
    then
        rm -rf /www/wwwroot/${domain}/html
        wget -q -P //www/wwwroot/${domain} https://raw.githubusercontent.com/mack-a/v2ray-agent/${branch}/fodder/blog/unable/html.zip > /dev/null
        unzip -o  /www/wwwroot/${domain}/html.zip -d /www/wwwroot/${domain}/html > /dev/null
    else
...
unInstall(){
    ...
    echoContent green " ---> BT用户伪装网站目录请自行删除"
}
...
updateV2RayAgent(){
    ...
    wget -P /etc/v2ray-agent/ -N --no-check-certificate "https://raw.githubusercontent.com/shyperwang/v2ray-agent/master/install_add_bt.sh" && chmod 700 /etc/v2ray-agent/install_add_bt.sh && vasma
}
...
# 脚本快捷方式
aliasInstall(){
    if [[ -f "/root/install_add_bt.sh" ]] && [[ -d "/etc/v2ray-agent" ]] && [[ ! -z `cat /root/install_add_bt.sh|grep "作者：mack-a"` ]]
    then
        mv /root/install_add_bt.sh /etc/v2ray-agent/install_add_bt.sh
        if [[ -d "/usr/bin/" ]] && [[ ! -f "/usr/bin/vasma" ]]
        then
            ln -s /etc/v2ray-agent/install_add_bt.sh /usr/bin/vasma
            chmod 700 /usr/bin/vasma
            rm -rf /root/install_add_bt.sh
        elif [[ -d "/usr/sbin" ]] && [[ ! -f "/usr/sbin/vasma" ]]
        then
            ln -s /etc/v2ray-agent/install_add_bt.sh /usr/sbin/vasma
